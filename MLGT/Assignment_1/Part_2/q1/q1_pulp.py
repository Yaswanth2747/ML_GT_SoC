...
def pulp_solver(A_matrix: numpy.array, c: numpy.array, b: numpy.array) -> (numpy.array, float): # type: ignore
    """
    ...
    x = numpy.array([po.LpVariable(f'x{i}', lowBound=0) for i in range(len(c))])
    prob = po.LpProblem("myProblem", po.LpMaximize)

    for i in range(len(c)):
        prob += numpy.dot(A_matrix[i], x) <= b[i]

    prob += numpy.dot(c, x)
    prob.solve()

    x = numpy.array([po.value(var) for var in x])
    opt_val = numpy.dot(c, x)
